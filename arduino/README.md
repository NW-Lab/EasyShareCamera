...
| 高輝度赤色LED | 1-2 | 5mm、順方向電圧2.0V（トリガー用） |
| 高輝度白色LED | 3-5 | 5mm、順方向電圧3.0-3.5V（照明用） |
| 抵抗 220Ω | 4-7 | LED電流制限用（各LEDに1個） |
| ブレッドボード | 1 | プロトタイピング用 |
...
2. **LEDの配置**
   - **赤色LED**: iPhoneのカメラから見える位置に配置（画面中央）
   - **白色LED**: 撮影領域全体を照らすように配置（複数個推奨）
   - 白色LEDは十分な明るさが必要（240fpsは露光時間が短い）
...
2. 水滴を落とす
3. フォトカプラが水滴を検知
4. 赤色LEDが点灯（トリガー用、5秒間）
5. 白色LEDが点灯（照明用、5秒間）
6. iPhoneが自動的に録画開始
7. 録画完了後、全LEDが消灯
8. 1秒のクールダウン後、次の撮影が可能
